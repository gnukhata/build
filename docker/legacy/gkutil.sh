...
psql -c "create role gkadmin with login"
echo 'Role gkadmin created'

psql -c "alter role gkadmin createdb;"
echo 'Granted database creation privileges to role sttpadmin'

psql -c "grant all privileges on database template1 to gkadmin;"
echo 'Granted all privileges to role gkadmin'

psql -c "create database gkdata"
